...
def uz(update: Update, context: CallbackContext) -> None:
    update.message.reply_text('salom')

def juft_toq(update: Update, context: CallbackContext) -> None:
    son = int(update.message.text)
    if son % 2 == 0:
        update.message.reply_text('Juft')
    else:
        update.message.reply_text('Toq')
...
def main() -> None:
    updater = Updater("6156151673:AAGxZtz3UqM3PBO4wC0IXGqU0e_aZ0mP2OI")
    ...
    updater.dispatcher.add_handler(CommandHandler("help", help_command))
    updater.dispatcher.add_handler(MessageHandler(get_greeting_filter('salom'), uz))
    updater.dispatcher.add_handler(MessageHandler(Filters.update.message & Filters.text, juft_toq))
